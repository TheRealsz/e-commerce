            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex p-14 flex-col items-start gap-3 outline-none bg-white rounded-2xl shadow-2xl">
                <div className="flex justify-center items-center gap-6">
                        ...
                        <div className="flex flex-col justify-center items-center h-full gap-4 self-stretch">
                            {
                                imgArray?.map((img, i) => (
                                    <div className={`flex p-3 justify-end items-end gap-3 ${currentIndex == i ? "w-36 h-36" : "w-32 h-32"}`}>
                                        <img src={`${baseUrl}${imgArray[i]}`} alt="" className="w-full h-full rounded-2xl hover:scale-105 transition-all" onClick={() => setCurrentIndex(i)} />
                                    </div>
                                ))
                            }
                        </div>
                    ...
                    <div className="w-modalPhoto h-modalPhoto relative">
                        <img src={`${baseUrl}${imgArray[currentIndex]}`} alt="" className="flex w-full h-full justify-center items-center shrink-0 rounded-lg" />
